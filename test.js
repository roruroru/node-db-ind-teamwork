...
const extractProductDetailsMost = async (url) => {
    ...
    let screenSizeInch = 0;
    let resolution = '';
    let angleOfView = '';
    let reactionTime = 0;
    const selector = $('#product-attribute-specs-table tbody th').each((_, el) => {
        const $child = $(el);
        // console.log($child.text());
        if ($child.text() === 'Размер на екрана ') {
            screenSizeInch = +$($child).next().text().split('"')[0];
        } else if ($child.text() === 'Разделителна способност') {
            resolution = $($child).next().text();
        } else if ($child.text() === 'Ъгъл на видимост') {
            angleOfView = $($child).next().text();
        } else if ($child.text() === 'Време за реакция') {
            reactionTime = +$($child).next().text().split(' ')[0];
        }
    });
